  ...
  function renderTopics(){
    ...
    document.querySelectorAll('[data-topic]').forEach(cb => {
      cb.addEventListener("change", (e) => {
        ...
        RP.api.setTopicDone(id, idx, e.target.checked);
        e.target.closest(".topic-row").classList.toggle("done", e.target.checked);
        renderProgress();
        RP.showToast(e.target.checked ? "Topic marked complete" : "Topic marked incomplete");
      });
    });
  }
